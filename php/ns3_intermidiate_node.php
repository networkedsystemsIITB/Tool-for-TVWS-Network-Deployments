	...
	<div class='navbar navbar-default navbar-static-top'>
		...
		<form action="ns3_simulation_run.php" method="post" >
			<div class='col-md-4'>
				...
				Both&nbsp;&nbsp;<input type="radio" name="compare" value="both" onclick="show_both();">
				</label>
				</ul>
				</div>
...
<?php
	...
	$filename = 'csv/inter_place_pop_lat_long_alt_bw.csv';
	if (file_exists($filename)) {
	    chmod("csv/inter_place_pop_lat_long_alt_bw.csv",0777);
	}

	$filename1 = 'csv/place_pop_lat_long_alt_bw_range_cap.csv';  // no delete
	if (file_exists($filename1)) {
	    chmod("csv/place_pop_lat_long_alt_bw_range_cap.csv",0777);
	}
	$filename2 = 'csv/inter_place_pop_lat_long_alt_bw_range_cap.csv';  
	if (file_exists($filename2)) {
	    chmod("csv/inter_place_pop_lat_long_alt_bw_range_cap.csv",0777);
	}
	$filename3 = 'csv/line_place_pop_lat_long_alt_bw_range_cap.csv'; 
	if (file_exists($filename3)) {
	    chmod("csv/line_place_pop_lat_long_alt_bw_range_cap.csv",0777);
	}
	
	$filename4 = 'csv/place_pop_lat_long_alt_bw.csv';
	if (file_exists($filename4)) {
	    chmod("csv/place_pop_lat_long_alt_bw.csv",0777);
	}

   shell_exec("bash to_delete2.sh");
   ...
   $filename_inter = 'csv/inter_place_pop_lat_long_alt_bw.csv';
	if (file_exists($filename_inter)) {
	    chmod("csv/inter_place_pop_lat_long_alt_bw.csv",0777);
	}
  ...
  //chmod("csv/place_pop_lat_long_alt_bw.csv",0777);
   ...
   $filename_inter1 = 'csv/inter_place_pop_lat_long_alt_bw_range_cap.csv';  // no delete
	if (file_exists($filename_inter1)) {
	    chmod("csv/inter_place_pop_lat_long_alt_bw_range_cap.csv",0777);
	}
  ...
  // chmod("csv/inter_place_pop_lat_long_alt_bw_range_cap.csv",0777);
   ...
   $filename_inter2 = 'csv/place_pop_lat_long_alt_bw_range_cap.csv';  // no delete
	if (file_exists($filename_inter2)) {
	    chmod("csv/place_pop_lat_long_alt_bw_range_cap.csv",0777);
	}
   ...
   // here call for the line creation
    ...
    $filename_inter3 = 'csv/line_place_pop_lat_long_alt_bw_range_cap.csv';  // no delete
	if (file_exists($filename_inter3)) {
	    chmod("csv/line_place_pop_lat_long_alt_bw_range_cap.csv",0777);
	}
   // chmod("csv/line_place_pop_lat_long_alt_bw_range_cap.csv",0777);
		...
		(function (marker, data)
		{
			...
			google.maps.event.addListener(marker, "mouseover", function (e)
			{
			   ...
			   else if(t.charAt(0)=="I" && t.charAt(1)=="N")
			   {
				...
				//infoWindow.setContent("<h6>Place: " + data.place );//
...
function show_tdma() { 
	...
	document.getElementById("channel_area").style.display = 'block';
...
function show_both() { document.getElementById("csma_area").style.display = 'block';
							  document.getElementById("tdma_area1").style.display = 'block';
							  document.getElementById("tdma_area2").style.display = 'block';
							  document.getElementById("tdma_area3").style.display = 'block';
							  document.getElementById("tdma_area4").style.display = 'block';
