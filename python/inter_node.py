...
for i in range (0, len(data)):
	linear_range.append(math.sqrt((float(data[i][6]) * math.pow(10, 6))/3.141))  #in metres
...
def place_intermediate_nodes(arr, b):
	...
	for i in range(0, len(arr)):
		...
		if (linear_range[i] <= min(arr[i])):
			...
			inter_node_points = calc_distance(i, float(data[i][2]), float(data[i][3]), float(data[b[i]][2]), float(data[b[i]][3]), arr[i][b[i]], int(linear_range[i]-1000),float(data[i][4]))
			dictMap[i] = b[i]
			dictMap[b[i]] = i
...
def calc_distance(nodeID, start_lat, start_long, end_lat, end_long, end_distance, inter_distance, lat_in):#=3000):
	...
	for i in range (inter_distance, int(end_distance), inter_distance):
		...
		#f.write("IN%d_" % i)
		f.write("IN%d_%d,1,%f,%f,%f,0.1\n" % (nodeID, j, latLongPoints[j][0], latLongPoints[j][1], lat_in))
